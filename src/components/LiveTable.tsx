  ...
  return (
    <>
      <div className="toolbar">
        <select className="btn" onChange={e=>setMode(e.target.value as any)} value={mode}>
        ...
        <table>
          ...
          <tbody>
            {rows.map((r:any, i:number)=>{
              ...
              const trClass = `${i<3?'top3':''} ${String(team).includes(club||'')?'me':''}`

              return mode==='full' ? (
                <tr key={i} className={trClass}>
                  <td>{rank}</td><td className="team">{team}</td>
                ...
                <tr key={i} className={trClass}>
                  <td>{rank}</td><td className="team">{team}</td><td>{pts}</td>
                </tr>
              )
            })}
          </tbody>
